...
分析RAY 的RDT在哪些类和方法中调用了英伟达的API，如何调用的，整理成一个列表，要包含RAY的类和方法，英伟达的API，以及英伟达的API属于哪个软件和部件，使用该API完成了什么样的能力，然后分析umdk（这是一个华为的灵衢内存语义开发包，源代码放在了D盘，路径：D:\C++\umdk-master，已经在另一个opencode窗口对其init了）提供了哪些API，哪些与英伟达的api对应，也需要整理成一列，一列是英伟达的api，一列是UMDK的api，一列分析两者差异，最后一列要说明二者如果存在差异该如何通过UMDK替换和适配，使得RAY的RDT能力在使用华为的灵衢总线超节点集群上也能够像英伟达一样有着GPU数据直通的能力，能够有着NPU数据直通的能力，最后给出具体的设计方案，包含如何修改RAY的哪些代码给出具体类和方法，使用灵衢的UMDK哪些API，以及灵衢UMDK有哪些能力不足需要进一步，给出具体改进方案，最后整理成md，输出到桌面
## 一、RAY RDT概述
